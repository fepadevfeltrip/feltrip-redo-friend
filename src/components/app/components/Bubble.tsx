...
const extractReadableFromJson = (text: string): string | null => {
  try {
    ...
    // Known keys from the Bedrock agent responses
    const readableKeys = [
      "diagnostic_text",
      "poetic_proposition",
      "emotional_status",
    ...
    // Extract emotional_status as a title
    if (obj.emotional_status) {
      parts.push(`### ✦ ${obj.emotional_status}`);
    }

    // Extract readable text fields
    for (const key of readableKeys) {
      if (key === "emotional_status") continue;
      if (obj[key] && typeof obj[key] === "string") {
        parts.push(obj[key]);
      }
    }

    // Fallback: if no known keys found, extract ALL string values from the object
    if (parts.length === 0) {
    ...
    if (parts.length > 0) {
      ...
      const suffix = text.substring(end + 1).trim();
      return [prefix, ...parts, suffix].filter(Boolean).join("\n\n");
    }

    // If it's a JSON object but has no readable strings, suppress it entirely
    return "";
  } catch {
...
const FormattedText: React.FC<{ text: string }> = ({ text }) => {
  ...
  let filtered = text
    ...
    .replace(/<\/?json[^>]*>/gi, "")
    // Remove stray JSON-like fragments: {"key": "value", ...}
    .replace(/\{["\s]*\w+["\s]*:\s*["\[{][\s\S]{0,500}?\}/g, (match) => {
      // Only remove if it looks like raw JSON, not like a template literal
  ...
  // If the remaining text looks like raw JSON, extract readable content
  const jsonExtracted = extractReadableFromJson(filtered);
  if (jsonExtracted) {
    filtered = jsonExtracted;
  }
      ...
      <div className="absolute inset-0 z-20 flex items-center justify-center">
        <div className="mx-4 w-full max-w-sm space-y-4 rounded-3xl border-2 border-boba-coral/30 bg-white p-6 text-center shadow-2xl dark:bg-boba-darkCard sm:p-8">
          ...
          <div className="space-y-1 px-2 text-left text-sm text-gray-600 dark:text-gray-300">
            ...
            <p>• {lang === "es" ? "24h de chat con Cultii AI" : lang === "en" ? "24h chat with Cultii AI" : "24h de chat com Cultii AI"}</p>
          </div>

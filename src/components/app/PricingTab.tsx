...
const PricingTab = () => {
  ...
  // Pegando os dados do usuário para verificar o plano atual
  const { isPremium, isExplorer, isFree, isPremiumCompany } = useUserTier();

  // Injetando os preços dinamicamente
  const t = {
    ...baseTexts,
  ...
  const handlePlanClick = async (key: PriceKey) => {
    ...
    try {
      ...
      // Em IAP, o loading é tirado pelo listener ou por erro.
    ...
    <div className="p-4 space-y-6 max-w-lg mx-auto pb-20">
      ...
      <div className="space-y-6">
        {t.plans.map((plan: any, i: number) => {
          ...
          const isPaidCard = !isFreeCard;
          const key: PriceKey = i === 1 ? "personal_map" : "explorer";

          // Lógica para saber se este é o plano atual do usuário
          const isCurrentPlan = (i === 0 && isFree) || (i === 1 && isPremium && !isExplorer) || (i === 2 && isExplorer);
          const isPremiumCompanyPlan = i === 2; // Supondo que empresa seja explorer
          ...
          return (
            ...
            >
              ...
              <CardContent className="pt-8 pb-6 px-6">
                ...
                {isFreeCard && isFree && (
                  <Badge variant="secondary" className="w-full flex items-center justify-center text-center py-3 px-4 text-xs uppercase tracking-widest whitespace-normal break-words h-auto min-h-[3.5rem] leading-tight rounded-xl">
                    ✓ {plan.cta}
                  </Badge>
                )}
              </CardContent>
      ...
      <button
        ...
        className="w-full mt-8 rounded-2xl border border-dashed border-primary/40 bg-primary/5 p-4 flex flex-col items-center justify-center space-y-2 hover:bg-primary/10 transition-colors"
      >
        <div className="flex items-center justify-center gap-3 w-full px-2">
          <Gift className="h-6 w-6 text-primary shrink-0" />
          <span className="text-xs sm:text-sm font-bold text-foreground text-left whitespace-normal break-words leading-tight">
            {t.squadTitle}
          </span>
        </div>
        <p className="text-[10px] font-bold text-primary uppercase tracking-widest text-center whitespace-normal break-words w-full px-2">
          {t.squadCta}
        </p>
      </button>

...
UNION ALL
 SELECT 'language_studio'::text AS activity_type,
    ul.id AS record_id,
    ul.user_id,
    COALESCE(p.full_name, 'Anônimo'::text) AS client_name,
    p.city AS client_city,
    p.user_tier,
    NULL::text AS session_city,
    ul.ai_function AS archetype,
    NULL::text AS portal,
    NULL::text AS emotional_status,
    ul.description AS poetic_proposition,
    NULL::text AS language,
    (ROUND(ul.duration_seconds / 60.0, 1) || ' min')::text AS gem_name,
    NULL::text AS gem_description,
    NULL::text AS gem_category,
    ul.created_at
   FROM usage_log ul
